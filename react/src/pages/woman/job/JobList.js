...
class JobList extends React.Component {
  ...
  render() {
    ...
    // props = { images, name, jobStatus, pax, state, time, location }

    const { classes, image, name, time, pax, location } = this.props;
      ...
      <div className={classes.root}>
        ...
        <Grid container alignItems="center" className={classes.container}>
          ...
          <Grid item xs={5} className={classes.alignLeft}>
            ...
            <h6 className={classes.des}>{pax}pax | {time}</h6>
            <h6 className={classes.des}>{location}</h6>
          </Grid>
          <Grid item xs={3} className={classes.alignRight}>
            <h6 className={classes.des}>
              <span className={classes.label} style={{backgroundColor : jobStatus.color}}>{jobStatus.label}</span>
            </h6>
          </Grid>
        </Grid>
      </div>
