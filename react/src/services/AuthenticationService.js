...
const AuthenticationService = {
    login: function(value){
        return axios.post(Backend.apiUrl + Backend.authUrl + "login", qs.stringify(value),).then(res => {
            if (res.data.status) {
    ...
    signUp: function(value){
        return axios.post(Backend.apiUrl + Backend.authUrl + "sign-up", qs.stringify(value)).then(res => {
            if (res.data.status) {
                Cookies.set("nomi-token", res.data.session);
            }
            return res.data;
